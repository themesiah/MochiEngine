...
AnimationFactory::AnimationFactory(std::shared_ptr<PackCatalog> catalog) : mCatalog(catalog)
{
}
...
{
    std::string normalizedPath = NormalizePath(animationDataPath);
...
{
    std::string normalizedPath = NormalizePath(animationPath);
    if (mAnimationDataMap.find(normalizedPath) != mAnimationDataMap.end())
    {
